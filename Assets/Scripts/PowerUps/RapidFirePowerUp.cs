...
public class RapidFirePowerUp : MonoBehaviour , IPowerUp
{
     [SerializeField]
     float coolDownTime;
     [SerializeField]
    ...
    void RapidShooting()
    {
        ...
        if (T_ShootDelay >= shootDelay)
        {
            ...
            b.GetComponent<Bullet>().SetRapidBullet();
            totalBulletShoots++;
            print("Power Up Rapid Shooting ...");

            if (totalBulletShoots > 15)
            {
                Deactivate();
            }
